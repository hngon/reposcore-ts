...
<!-- DOC_LIST_START -->
- [ts-bun-guide.md](./ts-bun-guide.md): 타입스트립트 및 실행 환경 bun에 대한 안내
- [ts-convention.md](./ts-convention.md): TypeScript 코딩 컨벤션 가이드
<!-- DOC_LIST_END -->

---

> **아래 상황이 발생하면 반드시 스크립트를 통해 목록을 자동 갱신해줘야 합니다.**
>
...
> ```
>
> 스크립트 위치: `tools/update_readme.py`
>
> ```text
> 참고사항: 실행 명령어는 현재 작업중인 디렉터리에 따라 유동적으로 변동될 수 있습니다.
> 예시) Root가 아닌 docs/에 위치할 시: python ../tools/update_readme.py
> ```
>
> **수작업으로 문서 목록 갱신하지 마세요.**  
> **docs/README.md 파일을 직접 수정하지 마세요.**

---

## docs/README.md 파일 수정 가이드

> `docs/README.md`를 직접 수정하지 마세요.

`docs/README.md`를 수정하려면 먼저 `docs/templates/README-template.md`를 업데이트한 후, 스크립트를 실행하여 `docs/README.md`를 자동으로 갱신하세요.

### 수정 방법

1. `docs/templates/README-template.md`를 업데이트합니다.
2. 루트 디렉터리에서 `python tools/update_readme.py`를 실행합니다.
